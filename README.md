...
<details>
<summary>連線 Battle.net 時顯示「無法認證」怎麼辦？</summary>

這表示遊戲已經開啟，但 Battle.net 拒絕了這個帳號的登入，和多開工具無關。請依序確認：

1. **帳號是否有綁手機驗證器（OTP）**：有綁的帳號不能用帳號密碼方式登入。請在帳號設定勾選「使用 Token 登入（支援 OTP 驗證器）」，按「取得 Token」並在手機上核准。
2. **密碼是否正確**：最近改過密碼的話，請在帳號設定重新輸入，可按欄位右側的眼睛確認有沒有打錯。
3. **登入帳號（Email）是否正確**：確認沒有多打空白或打錯字。
4. **Token 是否失效**：使用 Token 登入的帳號，如果停用或重新綁定過驗證器，請按「清除」後重新取得 Token。
5. **是否需要額外的安全驗證**：先用 Battle.net 啟動器正常登入一次這個帳號，完成畫面要求的驗證後再試。

都確認過仍然失敗，請到 [Issues](https://github.com/MoroseDog/D2RNexus/issues) 回報，並附上日誌內容（上方選單「說明 → 開啟日誌資料夾」）。
</details>
