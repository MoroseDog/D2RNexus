...
| Display settings | Only rewrites the window size and frame cap in the game's settings file `Settings.json`, leaves everything else alone, and makes a backup before the first change. The game limits frames at whichever is higher, the cap or the target frame rate of dynamic resolution scaling, so a target above the cap is brought down to the same number |
