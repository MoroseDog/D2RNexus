...
```mermaid
flowchart LR
    A["'선택한 계정 실행'을 누름"] --> B{"이 계정은 어떻게 로그인하나요?"}
    B -->|계정과 비밀번호| C["게임이 실행되고 스스로 로그인"]
    C --> D["기다릴 필요 없이 다음 계정 실행"]
    B -->|토큰| E["토큰을 기록하고 게임 실행"]
    E --> F["'아무 키나 누르십시오' 화면에서 대기"]
    F --> G["아무 키나 누르면 게임이 로그인"]
    G --> H["토큰을 읽어 가면 다음 계정 실행"]
```

Windows에는 토큰이 한 번에 하나만 저장되므로 토큰 계정 두 개를 동시에 실행할 수 없습니다. **앞 게임의 창에서 키를 누르기 전까지 뒤의 계정은 계속 기다립니다.** 활동 기록에 무엇을 얼마나 기다리는지 표시됩니다. 계정과 비밀번호로 로그인하는 계정은 스스로 로그인하므로 영향을 받지 않습니다.
...
계정이 많으면 목록이 길어집니다. 빼고 싶은 계정은 해당 줄에서 마우스 오른쪽 버튼을 눌러 '이 목록에서 숨기기'를 선택하거나, 계정 설정에서 '미니 창에 표시'를 끄세요. 목록 아래에는 표시하지 않은 계정이 몇 개인지, 그중 몇 개가 실행 중인지 표시되며, 옆의 화살표를 누르면 펼쳐지고 한 번 클릭하면 다시 목록에 넣습니다. 숨긴 계정도 평소처럼 실행되고 단축키도 그대로 작동합니다.
...
<details>
<summary>아시아는 되는데 유럽이나 아메리카에서는 서버에 연결할 수 없다고 나옵니다</summary>

계정과 비밀번호로 로그인할 때 생기는 문제이며, 그 계정을 토큰 로그인으로 바꾸면 정상적으로 됩니다. 아시아는 영향을 받지 않습니다.

계정 설정에서 '로그인 지역'을 유럽이나 아메리카로 바꾸고 '토큰으로 로그인'을 체크한 뒤 '토큰 받기'를 눌러 휴대폰에서 한 번 승인하세요. **토큰은 지역별로 다르므로** 지역을 바꾸면 다시 받아야 합니다. 받은 토큰이 로그인 지역과 다르면 활동 기록이 알려 줍니다.
</details>
...
<details>
<summary>토큰으로 로그인한 계정이 '아무 키나 누르십시오' 화면에서 멈춥니다</summary>

게임 자체의 동작입니다. 토큰으로 실행하면 게임이 시작 화면에서 멈추고, **그 창에서 아무 키나 눌러야** 로그인을 시작합니다. 계정과 비밀번호로 로그인하는 계정은 알아서 로그인합니다.

토큰 계정이 여러 개라면 이 단계가 더 중요합니다. Windows에는 토큰이 한 번에 하나만 저장되므로 **앞 계정이 로그인해야 다음 계정이 실행됩니다**. 키를 누르지 않으면 뒤의 계정은 계속 기다립니다(활동 기록에 무엇을 얼마나 기다리는지 표시됩니다). 창이 뜰 때마다 아무 키나 눌러 주면 나머지가 이어서 실행됩니다.
</details>
...
'화면 및 성능'에서 설정한 **창 해상도**와 **프레임 제한**만 기록하며, 다른 게임 설정(그래픽 품질, 음량, 자동 지도, 키 설정 등)은 전혀 건드리지 않습니다. 게임은 '프레임 제한'과 '동적 해상도 조절'의 '목표 프레임' 중 더 높은 쪽으로 프레임을 제한하므로, 목표 프레임이 제한보다 높으면 Nexus가 같은 값으로 낮춥니다.
...
<details>
<summary>프레임 제한을 설정했는데 실제 프레임이 다릅니다</summary>

먼저 게임 표시 설정의 **수직 동기화**를 확인하세요. 켜져 있으면 실제 프레임은 모니터 주사율을 나눈 값에만 맞춰집니다. 60Hz 화면이라면 60, 30, 20, 15이므로 36이나 45로 설정해도 30으로 돌아갑니다. 숫자 그대로 적용하려면 수직 동기화를 끄세요.

또 하나는 '동적 해상도 조절'의 **목표 프레임**입니다. 게임은 둘 중 더 높은 값으로 프레임을 제한합니다. v0.4.3부터 Nexus가 필요할 때 제한과 함께 낮추며, 이전 버전에서는 게임에서 직접 확인해 주세요.
</details>
...
<details>
<summary>여러 개를 켜면 버벅이거나, 한 번에 너무 많이 켜면 실행되지 않습니다</summary>

Nexus는 게임을 실행해 줄 뿐 실행된 뒤에는 성능을 쓰지 않습니다. 버벅임은 게임 여러 개가 함께 돌아가서 생깁니다. 배경에 띄워 두는 계정은 세 가지를 모두 설정해야 효과가 있습니다.

1. **프레임 제한** 15 또는 30
2. **CPU 우선순위** '낮음'으로 설정해 주력 계정이 CPU를 먼저 쓰게 하기
3. **창 해상도** 1280x720

한 번에 여러 개를 켤 때 앞의 게임이 아직 불러오는 중인데 다음 게임이 겹친다면, '전역 설정 → 실행 및 진단'에서 '계정 간 실행 간격'을 5~10초로 바꾸세요.
</details>
